...
#include "../Headers/stack_cld.h" // push
...
void generatePNG(const T_list l, const char * filename){
	...
	for(i=1;aux;i++, aux = aux->pNext)
	{
		if(aux->pNext){
			fprintf(file, "\"ID_%04d\" [label = \"{<elt> %s  | <next> }\"];\n", i,toString(aux->data));
			fprintf(file, "\"ID_%04d\" : next -> \"ID_%04d\";\n", i,i+1);
		}else{
			fprintf(file, "\"ID_%04d\" [label = \"{<elt> %s  | <next> NULL }\"];\n", i,toString(aux->data));
		}		
	}
...
T_list changeBase(int value, int base, T_list s){
	while(value>0){	// Ajouter chaque element de la base nouveau
		push(value%base, &s);
